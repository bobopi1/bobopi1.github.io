(function () {
  ...
  function renderBuilderControls() {
    ...
    const addonControls = document.querySelector("[data-builder-addon-controls]");
    ...
    if (addonRow) {
      if (productAddonProducts.length) {
        ...
        addonRow.innerHTML = visibleAddons.map((addon) => `
        ...
        if (addonControls) {
          addonControls.hidden = !hasMultiplePages;
          addonControls.innerHTML = hasMultiplePages ? `
            <button class="addon-carousel-button" type="button" data-builder-addon-prev aria-label="Voir les compléments précédents" ${builderState.addonPage === 0 ? "disabled" : ""}>
              <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
                <path d="m15 6-6 6 6 6"></path>
              </svg>
            </button>
            <div class="addon-carousel-dots" aria-label="Pages de compléments">
              ${Array.from({ length: pageCount }, (_, index) => `
                <button class="addon-carousel-dot${index === builderState.addonPage ? " is-active" : ""}" type="button" data-builder-addon-page="${index}" aria-label="Voir les compléments ${index * cardsPerPage + 1} à ${Math.min((index + 1) * cardsPerPage, productAddonProducts.length)}" ${index === builderState.addonPage ? 'aria-current="true"' : ""}></button>
              `).join("")}
            </div>
            <button class="addon-carousel-button" type="button" data-builder-addon-next aria-label="Voir les compléments suivants" ${builderState.addonPage >= pageCount - 1 ? "disabled" : ""}>
              <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
        ...
        if (addonControls) {
          addonControls.hidden = true;
          addonControls.innerHTML = "";
        }
      }
    }
